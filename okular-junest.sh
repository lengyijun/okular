...
# Please `pacman -Syu` before running this script

OKULAR_DIR="$PWD"

APP=okular
BIN="$APP" #CHANGE THIS IF THE NAME OF THE BINARY IS DIFFERENT FROM "$APP" (for example, the binary of "obs-studio" is "obs")
DEPENDENCES="libproxy" #SYNTAX: "APP1 APP2 APP3 APP4...", LEAVE BLANK IF NO OTHER DEPENDENCIES ARE NEEDED
...
LIBSAVED="SAVELIBSPLEASE $lib_audio_keywords $lib_browser_launcher libxcb libuuid"
...
make
make DESTDIR="$DIR"/"$APP".AppDir/.junest install
cd $DIR
